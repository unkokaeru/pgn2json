...
def parse_pgn_to_json(pgn: str, POS_FLAGS: dict[str, str]) -> dict[str, any] | None:
    """
    ...
    for move in game.mainline_moves():
        ...
        if board.piece_type_at(move.from_square) == chess.PAWN and abs(move.from_square - move.to_square) == 16:
            ...
            move_info = {
                ...
                "comment": {
                    ...
                    "content": [
                    {
                        ...
                        {
                            "type": "text",
                            "text": extract_comment(game, move)
                        }
                        ]
                    }
                    ]
                } if extract_comment(game, move) else None
            }
        if 'p' in flags:
            move_info = {
                "color": "b" if board.turn == chess.WHITE else "w",
...
def main() -> None:
    ...
    pgn_files: list[str] = [f for f in os.listdir(PGN_LOC) if os.path.isfile(os.path.join(PGN_LOC, f)) and f.endswith(".pgn")]
